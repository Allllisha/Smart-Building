...
export default function ProjectEditor() {
  ...
  const [shadowRegulation, setShadowRegulation] = useState<ShadowRegulation>({
    targetArea: '',
    targetBuildings: '',
    measurementHeight: '',
    measurementTime: '',
  ...
  const [zoningInfo, setZoningInfo] = useState<ZoningInfo>({
    zoningType: '',
  ...
  const [administrativeGuidanceItems, setAdministrativeGuidanceItems] = useState<AdministrativeGuidanceItem[]>([])
  ...
  const [regulationDataCache, setRegulationDataCache] = useState<Map<string, any>>(new Map())
    ...
    switch (fieldName) {
      ...
      case 'maxHeight':
        if (value) {
          ...
          if (heightNum > 200) return '高さは200m以下で入力してください'
        }
        return ''
  ...
  // プロジェクト読み込み完了時に規制情報を自動取得（デバウンス付き）
  useEffect(() => {
    if (currentProject?.location.address && currentProject.location.address.trim()) {
      const currentAddress = currentProject.location.address.trim()
      ...
      // 入力中フラグを表示
      setShadowRegulation(prev => ({ ...prev, isLoading: true }))
      
      // 3秒待ってから検索を実行（長めに設定）
      debounceTimerRef.current = setTimeout(() => {
        // console.log('📍 住所入力完了、規制情報を取得中:', currentAddress)
        previousAddressRef.current = currentAddress
      
      // 規制情報取得を即座に開始（キャッシュ機能付き）
      const fetchRegulations = async () => {
        ...
        // 住所から地域特性を推定（即座に表示するため）
        const getRegionBasedDefaults = (address: string) => {
          // 住所から区を抽出
          const wardMatch = address.match(/([^市区町村]+[区])/);
          ...
          const regionDefaults = {
            '世田谷区': {
              shadow: {
                targetArea: '第一種・第二種低層住居専用地域',
                targetBuildings: '軒高7m超または地上3階以上',
                measurementHeight: '1.5m',
                measurementTime: '冬至日 午前8時〜午後4時',
                range5to10m: '3時間以内',
                rangeOver10m: '2時間以内',
                isLoading: true
              },
            ...
            '新宿区': {
              shadow: {
                targetArea: '第一種・第二種低層住居専用地域、第一種中高層住居専用地域',
                targetBuildings: '高さ10m超',
                measurementHeight: '4.0m',
                measurementTime: '冬至日 午前8時〜午後4時',
                range5to10m: '3時間以内',
                rangeOver10m: '2時間以内',
                isLoading: true
          ...
          return regionDefaults[ward] || regionDefaults['世田谷区']; // 世田谷区をデフォルトに
        };
        
        // 住所に基づく地域特性データで即座に表示
        const regionData = getRegionBasedDefaults(currentProject.location.address);
        setShadowRegulation(regionData.shadow);
        setZoningInfo(regionData.zoning);
        ...
        try {
          ...
          Promise.allSettled(searchPromises).then(results => {
            let updatedShadow = { ...regionData.shadow }
            let updatedZoning = { ...regionData.zoning }
            ...
            // 全体のWeb検索結果をまとめて保存
            const combinedResults = results.reduce((acc, { status, value }) => {
          ...
          // メインの統合検索も並行実行（バックアップ）
          const mainSearchResult = await fallbackSearch()
          let regulationData = regionData
          
          if (mainSearchResult.success && mainSearchResult.data) {
            regulationData = {
              shadow: {
                targetArea: mainSearchResult.data.sunlightRegulation?.targetArea || regionData.shadow.targetArea,
                targetBuildings: mainSearchResult.data.sunlightRegulation?.targetBuildings || regionData.shadow.targetBuildings,
                measurementHeight: mainSearchResult.data.sunlightRegulation?.measurementHeight || regionData.shadow.measurementHeight,
                measurementTime: mainSearchResult.data.sunlightRegulation?.timeRange || regionData.shadow.measurementTime,
                range5to10m: mainSearchResult.data.sunlightRegulation?.shadowTimeLimit || regionData.shadow.range5to10m,
                rangeOver10m: regionData.shadow.rangeOver10m,
                isLoading: false
              },
              zoning: {
                zoningType: mainSearchResult.data.urbanPlanning?.useDistrict || regionData.zoning.zoningType,
                buildingCoverageRatio: parseFloat(mainSearchResult.data.urbanPlanning?.buildingCoverageRatio?.match(/\d+/)?.[0] || regionData.zoning.buildingCoverageRatio),
                floorAreaRatio: parseFloat(mainSearchResult.data.urbanPlanning?.floorAreaRatio?.match(/\d+/)?.[0] || regionData.zoning.floorAreaRatio),
                heightLimit: mainSearchResult.data.urbanPlanning?.heightRestriction || regionData.zoning.heightLimit,
                heightDistrict: regionData.zoning.heightDistrict
              },
          ...
          // 取得した都市計画情報を自動でプロジェクトに反映
          if (regulationData.zoning.zoningType) {
            debouncedUpdateProject(currentProject.id, {
              siteInfo: { 
                ...currentProject.siteInfo, 
                zoningType: regulationData.zoning.zoningType,
  ...
  useEffect(() => {
    if (currentProject && !shadowRegulation.isLoading && shadowRegulation.targetArea) {
      const compliance = checkRegulationCompliance(currentProject.buildingInfo, shadowRegulation)
  ...
  }, [currentProject?.buildingInfo, shadowRegulation])
            ...
            <Grid container spacing={4}>
              <Grid size={{ xs: 12, md: 6 }}>
                ...
                <TextField
                  fullWidth
                  label="用途地域"
                  value={currentProject.siteInfo.zoningType ?? ''}
                  onChange={(e) =>
                    debouncedUpdateProject(currentProject.id, {
                      siteInfo: { ...currentProject.siteInfo, zoningType: e.target.value },
                    })
                  }
                  placeholder="例：第一種住居地域"
                  helperText={zoningInfo.zoningType ? `AI取得結果: ${zoningInfo.zoningType}` : "住所入力後にAIが自動取得します"}
                  margin="normal"
                />
                <TextField
                  fullWidth
                  label="建ぺい率 (%)"
                  type="number"
                  value={currentProject.siteInfo.buildingCoverage?.toString() ?? ''}
                  onChange={(e) =>
                    debouncedUpdateProject(currentProject.id, {
                      siteInfo: { ...currentProject.siteInfo, buildingCoverage: Number(e.target.value) || 0 },
                    })
                  }
                  onFocus={(e) => e.target.select()}
                  placeholder="60"
                  helperText={zoningInfo.buildingCoverageRatio !== 0 && zoningInfo.buildingCoverageRatio ? `AI取得結果: ${zoningInfo.buildingCoverageRatio}%` : "建築基準法による建築面積の敷地面積に対する割合"}
                  margin="normal"
                />
                <TextField
                  fullWidth
                  label="容積率 (%)"
                  type="number"
                  value={currentProject.siteInfo.floorAreaRatio?.toString() ?? ''}
                  onChange={(e) =>
                    debouncedUpdateProject(currentProject.id, {
                      siteInfo: { ...currentProject.siteInfo, floorAreaRatio: Number(e.target.value) || 0 },
                    })
                  }
                  onFocus={(e) => e.target.select()}
                  placeholder="200"
                  helperText={zoningInfo.floorAreaRatio !== 0 && zoningInfo.floorAreaRatio ? `AI取得結果: ${zoningInfo.floorAreaRatio}%` : "建築基準法による延床面積の敷地面積に対する割合"}
                  margin="normal"
                />
                <TextField
                  fullWidth
                  label="高さ制限"
                  value={currentProject.siteInfo.heightLimit ?? ''}
                  onChange={(e) =>
                    debouncedUpdateProject(currentProject.id, {
                      siteInfo: { ...currentProject.siteInfo, heightLimit: e.target.value },
                    })
                  }
                  placeholder="例：10m"
                  helperText={zoningInfo.heightLimit ? `AI取得結果: ${zoningInfo.heightLimit}` : "建築基準法・地域条例による高さの上限"}
                  margin="normal"
                />
                <TextField
                  fullWidth
                  label="高度地区"
                  value={currentProject.siteInfo.heightDistrict ?? ''}
                  onChange={(e) =>
                    debouncedUpdateProject(currentProject.id, {
                      siteInfo: { ...currentProject.siteInfo, heightDistrict: e.target.value },
                    })
                  }
                  placeholder="例：第二種高度地区"
                  helperText={zoningInfo.heightDistrict ? `AI取得結果: ${zoningInfo.heightDistrict}` : "高度地区の指定"}
                  margin="normal"
                />
                <Divider sx={{ my: 2 }} />
                ...
                <FormGroup>
                  {administrativeGuidanceItems.map((item) => {
                    // item.nameが文字列でない場合の処理
                    const itemName = typeof item.name === 'string' 
                      ? item.name 
                      : (typeof item.name === 'object' && item.name !== null) 
                        ? ((item.name as any).name || (item.name as any).title || String(item.name))
                        : String(item.name)
                    ...
                    return (
                      <Box key={item.id} sx={{ mb: 1 }}>
                        <FormControlLabel
                          control={
                            <Checkbox
                              checked={currentProject.siteInfo.administrativeGuidance[item.id] || false}
                              onChange={(e) =>
                                debouncedUpdateProject(currentProject.id, {
                                  siteInfo: {
                                    ...currentProject.siteInfo,
                              ...
                              color={item.isRequired ? 'error' : 'primary'}
                            />
                          }
                          label={
                            <Box>
                              ...
                              {item.description && (
                                <Typography variant="caption" display="block" color="text.secondary">
                                  {typeof item.description === 'string' 
                                    ? item.description 
                                    : (item.description as any)?.details || JSON.stringify(item.description)}
                                </Typography>
                              )}
                  ...
                  {administrativeGuidanceItems.length === 0 && (
              ...
              <Grid size={{ xs: 12, md: 6 }}>
                ...
                {shadowRegulation.isLoading ? (
                  ...
                  <Box sx={{ 
                    mt: 2, 
                    p: 3, 
                    bgcolor: shadowRegulation.targetArea ? 'primary.50' : 'grey.50', 
                    borderRadius: 2, 
                    border: '1px solid', 
                    borderColor: shadowRegulation.targetArea ? 'primary.200' : 'grey.200' 
                  }}>
                    <Typography variant="body2" sx={{ mb: 1 }}>
                      <strong>規制対象地域:</strong> {shadowRegulation.targetArea || '情報取得中...'}
                    </Typography>
                    <Typography variant="body2" sx={{ mb: 1 }}>
                      <strong>規制対象建築物:</strong> {shadowRegulation.targetBuildings || '情報取得中...'}
                    </Typography>
                    <Typography variant="body2" sx={{ mb: 1 }}>
                      <strong>測定面高さ:</strong> {shadowRegulation.measurementHeight || '情報取得中...'}
                    </Typography>
                    <Typography variant="body2" sx={{ mb: 1 }}>
                      <strong>測定時間帯:</strong> {shadowRegulation.measurementTime || '情報取得中...'}
                    </Typography>
                    <Typography variant="body2" sx={{ mb: 1 }}>
                      <strong>5-10m範囲:</strong> {shadowRegulation.range5to10m || '情報取得中...'}
                    </Typography>
                    <Typography variant="body2">
                      <strong>10m超範囲:</strong> {shadowRegulation.rangeOver10m || '情報取得中...'}
                    </Typography>
                    
                    {shadowRegulation.targetArea && (
                      <Typography variant="caption" color="text.secondary" sx={{ mt: 2, display: 'block' }}>
                        ※ この情報は{currentProject?.location.address ? `「${currentProject.location.address}」` : '指定された住所'}の自治体条例に基づき、AIが自動取得・解析したものです
                      </Typography>
                    )}
                  </Box>
                )}
